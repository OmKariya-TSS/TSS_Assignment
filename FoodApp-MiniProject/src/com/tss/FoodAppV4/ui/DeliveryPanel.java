...
import com.tss.FoodAppV4.exceptions.AgentNotAvailableException;
import com.tss.FoodAppV4.exceptions.RestaurantNotFoundException;
...
public class DeliveryPanel {
    ...
    public static int deliveryAgentCount = 1;

    public DeliveryPanel(IDeliveryService deliveryService,
    ...
    public void addDeliveryAgent() {
        try {
            ...
            if (name.isEmpty() || phone.isEmpty())
                throw new ValidationException("Agent name and phone cannot be empty.");

            if (!phone.matches("\\d{10}"))
                throw new ValidationException("Phone must be 10 digits.");

            boolean phoneExists = restaurant.getAgents().stream()
                    .anyMatch(a -> a.getPhone().equals(phone));

            if (phoneExists)
                throw new ValidationException(
                        "Delivery agent with phone " + phone + " already exists.");

            System.out.print("  Password   : ");
            String password = input.getScanner().nextLine().trim();

            if (password.isEmpty())
                throw new ValidationException("Password cannot be empty.");

            DeliveryAgent agent =
                    new DeliveryAgent(deliveryAgentCount++, name, phone, password);
